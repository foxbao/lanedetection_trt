...
namespace util
{
    ...
    std::string get_file_name_no_extension(std::string fullname)
    {
        int idx_point;
        int idx_slash;
        idx_point = fullname.find_last_of('.');
        idx_slash = fullname.find_last_of('/');
        std::string file_name_no_extension = fullname.substr(idx_slash + 1, idx_point - idx_slash-1);
    ...
    size_t ImageBase::volume() const
    {
        ...
        return mDims.d[3] /* w */ * mDims.d[2] /* h */ * mDims.d[1];
    }
    ...
    void ImageBase::read(const PPM &i_ppm)
    {
        mPPM.magic=i_ppm.magic;
        mPPM.w=i_ppm.w;
        mPPM.h=i_ppm.h;
        mPPM.max=i_ppm.max;
        mPPM.buffer=i_ppm.buffer;
    }
    ...
    void ArgmaxImageWriter::processLane(const int *buffer)
    {
        mPPM.magic = "P6";
        mPPM.w = mDims.d[3];
        mPPM.h = mDims.d[2];
